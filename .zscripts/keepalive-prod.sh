...
while true; do
  ...
  now=$(date +%s)
  if [ "$code" != "200" ]; then
    if [ $((now - LAST_START)) -lt 30 ]; then sleep 10; continue; fi
    ...
    setsid bash -c 'cd /home/z/my-project/.next/standalone && NEXT_TELEMETRY_DISABLED=1 exec node server.js > /home/z/my-project/prod.log 2>&1' &
