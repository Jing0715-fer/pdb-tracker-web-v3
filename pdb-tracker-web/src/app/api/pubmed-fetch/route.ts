...
export async function POST(request: NextRequest) {
  try {
    ...
    const cachedArticles = await db.$queryRaw<any[]>`
      SELECT pubmed_id, title, authors, abstract, journal
      FROM pubmed_articles
      WHERE pubmed_id IN (${Prisma.join(validIds)})
    `;

    const cachedMap = new Map<string, PubmedArticle>();
    for (const a of cachedArticles) {
      cachedMap.set(a.pubmed_id, {
        pubmedId: a.pubmed_id,
        title: a.title || '',
        authors: a.authors || '',
        abstract: a.abstract || '',
        journal: a.journal || '',
      ...
      if (newlyFetched.length > 0) {
        try {
          ...
          for (const article of newlyFetched) {
            await db.$executeRaw`
              INSERT OR REPLACE INTO pubmed_articles (pubmed_id, title, authors, abstract, journal, pub_year, fetched_at)
