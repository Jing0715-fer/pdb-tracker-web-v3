  ...
  try {
    ...
    for (const s of structures) {
      const methodLabel = s.method?.includes('CRYO') ? 'Cryo-EM'
        ...
        : s.method || 'Unknown';
      const ifNote = s.journalIf ? ` (IF: ${s.journalIf.toFixed(1)})` : '';
      activities.push({
        id: `struct-${s.pdbId}`,
        type: 'new_structure',
    ...
    // 2. New papers — from PubMedArticle
    const papers = await db.pubMedArticle.findMany({
      select: {
        pubmedId: true,
        title: true,
        journal: true,
    ...
    // 4. Published reports — from WeeklyReport
