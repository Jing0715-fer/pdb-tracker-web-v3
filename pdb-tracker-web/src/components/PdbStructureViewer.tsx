...
function ColorPickerPopup({
  ...
  useEffect(() => {
    if (anchorRef?.current) {
      ...
      setPos({ top: rect.bottom + 4, left: Math.max(4, rect.left - 60) });
    }
  }, [anchorRef]);
...
function ChainRowItem({
  ...
  const isColorPickerOpen = colorPickerTarget?.type === 'entity' && colorPickerTarget?.key === chainKey;
  ...
  return (
    ...
    >
      ...
      {isColorPickerOpen && (
        <ColorPickerPopup
          currentColor={color}
          onColorChange={(newColor) => onEntityColorChange(chainKey, newColor)}
...
function LigandRowItem({
  ...
  const isColorPickerOpen = colorPickerTarget?.type === 'ligand' && colorPickerTarget?.key === code;
  const colorDotRef = useRef<HTMLButtonElement>(null);
  ...
  return (
    <div className="relative">
      ...
      {isColorPickerOpen && (
        <ColorPickerPopup
          currentColor={color}
          onColorChange={(newColor) => onLigandColorChange(code, newColor)}
          onClose={closeColorPicker}
          anchorRef={colorDotRef}
        />
      )}
...
export function PdbStructureViewer({ pdbId, className = '', layout = 'stacked' }: PdbStructureViewerProps) {
  ...
  // Color picker state
  const [colorPickerTarget, setColorPickerTarget] = useState<{ type: 'entity' | 'ligand'; key: string } | null>(null);
              ...
              await plugin.managers.structure.component.add(
                {
                  ...
                  representation: representation === 'ball-stick' ? 'ball-and-stick' : representation === 'surface' ? 'surface' : 'cartoon',
                },
                hierarchy.structures
              );
            } catch (e) {
  ...
  // Uses applyTheme with StructureSelectionQuery (like v2) to color all atoms in matching chains.
  // This approach works even if per-chain representations haven't been fully built yet.
  useEffect(() => {
    const plugin = pluginRef.current;
    ...
    return () => { cancelled = true; };
  }, [entityColors, structureLoaded]);

  // ─── Apply ligand colors in Molstar ───────────────────────────────────
  // Uses applyTheme with StructureSelectionQuery (like v2) to color all matching ligand atoms.
  useEffect(() => {
    const plugin = pluginRef.current;
    if (!plugin || !structureLoaded) return;
  ...
  const handleScreenshot = useCallback(async () => {
    ...
    try {
      ...
      if (dataUrl && dataUrl[0]?.data) {
        ...
        link.download = `${pdbId}-screenshot.png`;
        link.click();
      }
    } catch (err) {
  ...
  const handleFocusOnTarget = useCallback(async (target: string, type: 'entity' | 'ligand') => {
    ...
    try {
      ...
      let expression: any;
      if (type === 'entity') {
        // target is entityKey like "7BV2.A"
      ...
      for (const structure of hierarchy.structures) {
        for (const component of structure.components) {
          ...
          for (const rep of component.representations) {
            try {
              await plugin.managers.structure.component.removeRepresentation(rep);
            } catch { /* ignore */ }
          }

          // Add new representation
          const label = component.cell?.obj?.label || '';
          const isLigand = label.startsWith('Ligand ');
          const isChain = label.startsWith('Chain ');

          if (isLigand) {
            await plugin.managers.structure.component.addRepresentation(component, { type: 'ball-and-stick' });
          } else if (isChain) {
            const reprType = type === 'ball-stick' ? 'ball-and-stick' : type === 'surface' ? 'surface' : 'cartoon';
            await plugin.managers.structure.component.addRepresentation(component, {
              type: reprType,
              ...(type === 'surface' ? { typeParams: { size: 0.8, opacity: 0.7 } } : {}),
            } as any);
          }
        }
      }

      // Re-apply colors after representation change
      // The color effects will handle this since representation change doesn't affect entityColors
    } catch (err) {
...
function PdbStructureViewerFullscreen({
  ...
  const changeRepresentation = useCallback(async (type: RepresentationType) => {
    ...
    try {
      ...
      const reprParams: Record<RepresentationType, { type: string; typeParams?: Record<string, unknown> }> = {
        cartoon: { type: 'cartoon' },
        'ball-stick': { type: 'ball-and-stick' },
        surface: { type: 'surface', typeParams: { size: 0.8, opacity: 0.7 } },
      };

      const param = reprParams[type];
      for (const comp of s.components) {
        await plugin.managers.structure.component.addRepresentation(comp, param as any);
      }
    } catch (err) {
  ...
  const handleToggleSpin = useCallback(() => {
    ...
    try {
      ...
      if (isSpinning) {
        ...
        plugin.canvas3d?.setSettings({ publish: { ...spin, rotate: { speed: 1, step: 0.5, pauseAfter: 0 } } });
      }
      setIsSpinning(!isSpinning);
    } catch { /* ignore */ }
  }, [isSpinning]);
  ...
  const handleResetCamera = useCallback(() => {
    pluginRef.current?.managers?.camera?.reset(undefined, 600);
  }, []);
