...
function MethodDistributionBar({ evaluations, width = 200, height = 12 }: {
  ...
  const segments = useMemo(() => {
    ...
    evaluations.forEach(e => {
      ...
      // Count by first PDB structure method
    ...
    const total = cryoem + xray + nmr + other || 1;
    return [
      { count: cryoem, pct: cryoem / total, color: '#2d8f8f', label: 'Cryo-EM' },
      { count: xray, pct: xray / total, color: '#7c5cbf', label: 'X-ray' },
      { count: nmr, pct: nmr / total, color: '#c9872e', label: 'NMR' },
      { count: other, pct: other / total, color: '#6b7280', label: 'Other' },
    ];
  }, [evaluations]);
        ...
        {segments.map((seg, i) => {
          ...
          return (
            <motion.rect
              ...
              width={segWidth}
              height={height}
              rx={i === 0 || i === segments.filter(s => s.count > 0).length - 1 ? height / 2 : 0}
              fill={seg.color}
              opacity={0.85}
              initial={{ width: 0 }}
