    ...
    >
      <div className="gradient-border-inner bg-claude-surface dark:bg-[#242220] p-3 sm:p-4 claude-card-shadow transition-transform duration-200 min-w-0 h-full flex flex-col">
        <div className="flex items-start justify-between mb-1.5 sm:mb-2 min-h-[36px] gap-2">
          ...
          <div className="hidden sm:flex items-center justify-center h-[38px] min-w-0 flex-shrink">{children ?? <div className="h-[38px]" />}</div>
        </div>
...
// ─── IF Distribution Horizontal Bar ───────────────────────────────────────────

function IfDistributionBar({ entries, width = 100, height = 10 }: {
  entries: PdbEntry[];
  width?: number;
  height?: number;
}) {
  ...
  const segments = useMemo(() => {
    ...
    const total = top + high + mid + low || 1;
    return [
      { count: top, pct: top / total, color: '#dc2626', label: 'Top' },
      { count: high, pct: high / total, color: '#ea580c', label: 'High' },
      { count: mid, pct: mid / total, color: '#16a34a', label: 'Mid' },
      { count: low, pct: low / total, color: '#6b7280', label: 'Low' },
    ];
  }, [entries]);
      ...
      <div className="flex items-center gap-2 mt-0.5">
        {segments.filter(s => s.count > 0).map((seg, i) => (
          <div key={i} className="flex items-center gap-0.5">
            ...
            <span className="text-[8px] text-claude-text-muted font-mono">{seg.label} {seg.count}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 p-2 sm:p-3 card-shimmer [grid-auto-rows:1fr] min-w-0">
      ...
      <StatCard
        ...
        borderColor="#c9872e"
      >
        <IfDistributionBar entries={entries} width={100} height={8} />
      </StatCard>
    </div>
  );
}
