...
import { LiteratureContent } from '@/components/literature';
...
const MODE_TABS: { mode: Mode; label: string; icon: React.ReactNode; shortcut: string }[] = [
  { mode: 'weekly', label: 'Weekly', icon: <Database className="h-3.5 w-3.5" />, shortcut: '1' },
  { mode: 'evaluation', label: 'Evaluation', icon: <FlaskConical className="h-3.5 w-3.5" />, shortcut: '2' },
  { mode: 'literature', label: 'Literature', icon: <BookOpen className="h-3.5 w-3.5" />, shortcut: '3' },
];
...
export default function PdbTracker() {
  ...
  const litHasActiveFilters = litSelectedDate !== null;
  ...
  // ─── Render: Literature Content ──────────────────────────────────────────

  const renderLiteratureContent = () => {
    // Filter papers by reading list
    const filteredLitPapers = litReadingListFilter
      ? (() => {
          const list = readingListState.lists.find(l => l.id === litReadingListFilter);
          if (!list) return litPapers;
          return litPapers.filter(p => list.paperPmids.includes(p.pmid));
        })()
      : litPapers;
    ...
    return (
      <LiteratureContent
        ...
        onClearAllFilters={() => {
          handleLitClearAllFilters();
          setLitReadingListFilter(null);
          setLitTagFilter(null);
        }}
  ...
  // ─── Responsive Detail Panel Wrapper ─────────────────────────────────────

  // On mobile (<768px), detail panels render as full-screen overlays.
  // On desktop (>=768px), they render as inline panels with responsive width.
    ...
    <div className="h-full w-full flex flex-col bg-claude-bg overflow-hidden">
      ...
      <BreadcrumbNav
        ...
        evalBatchName={evalBatches.find(b =>
          batchSubTargets[b.id]?.some(st => st.uniprotId === selectedEvalId)
        )?.name}
            ...
            >
              ...
              <motion.div
                ...
                style={{
                  ...
                  background: 'linear-gradient(90deg, #c96442, #2d8f8f, #7c5cbf, #c9872e)',
                }}
              />
