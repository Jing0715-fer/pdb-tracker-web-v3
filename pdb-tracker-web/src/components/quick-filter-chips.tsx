...
const QUICK_FILTERS: QuickFilterConfig[] = [
  ...
  },
  {
    ...
    matchCount: (entries) => entries.filter(e => {
      ...
      return m.includes('CRYO-EM') || m.includes('ELECTRON MICROSCOPY') || e.isCryoem === 1;
    }).length,
    matchesEntry: (entry) => {
      const m = (entry.method || '').toUpperCase();
      return m.includes('CRYO-EM') || m.includes('ELECTRON MICROSCOPY') || entry.isCryoem === 1;
    },
  },
  {
    key: 'xray_only',
    ...
    matchCount: (entries) => entries.filter(e => {
      const m = (e.method || '').toUpperCase();
      return m.includes('X-RAY') || e.isXray === 1;
    }).length,
    matchesEntry: (entry) => {
      const m = (entry.method || '').toUpperCase();
      return m.includes('X-RAY') || entry.isXray === 1;
    },
  },
  {
    key: 'high_if',
