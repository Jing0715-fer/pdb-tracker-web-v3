...
export function applyAdvancedFilters(papers: LitPaper[], filters: AdvancedFilterState): LitPaper[] {
  ...
  if (filters.methods.length > 0) {
    result = result.filter(p => {
      ...
      return filters.methods.some(fm => paperMethods.includes(fm));
    });
  }
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        className="h-8 px-2 text-xs rounded-lg border border-claude-border dark:border-[#3d3832] bg-white dark:bg-[#242220] text-claude-text-secondary focus:outline-none focus:ring-2 focus:ring-claude-accent/30"
                      >
