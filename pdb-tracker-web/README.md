...
> **This is the Next.js (React) version** of PDB Tracker. The original Flask version is in the root directory.
...
PDB Tracker Web has three fully-featured modes accessible from the sidebar:
...
- **Feasibility Scoring**: Multi-factor scoring (resolution, identity, BLAST coverage, method diversity)
- **Radar Chart**: Visual scores across all evaluation factors
- **Domain Coverage**: Analyze coverage across protein domains
- **Ramachandran Plot**: Real phi/psi scatter from PDB coordinates
- **Validation Metrics**: Clash score, rotamer outliers, MolProbity score via RCSB Validation API
- **Batch Evaluation**: Compare multiple targets side-by-side
- **Gantt Timeline**: Structural coverage timeline across years
- **Scatter Plot**: Resolution vs. BLAST identity distribution
- **Report Generation**: Download structured evaluation reports

### 3. Literature Mode 📚
Manage and organize related scientific papers and citations.

- **Paper Management**: Track and organize PubMed articles
...
- **Advanced Filters**: Filter by tags, date range, journal, authors
- **Paper Detail Panel**: Full paper metadata with abstract and citation count
...
│   │   ├── notification-panel.tsx       # Notification bell
│   │   └── context-menu-overlay.tsx     # Right-click menu
│   │
│   ├── hooks/
│   │   ├── use-pdb-filters.tsx         # Filter state
...
| GET | `/api/evaluations/[uniprotId]` | Evaluation for a UniProt ID |
...
```bash
# pdb-tracker-web/.env.local (create if missing)
