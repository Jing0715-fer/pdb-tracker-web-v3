...
export async function POST(req: Request) {
  ...
  (async () => {
    ...
    const allIds = [...new Set([...pathAIds, ...pathBIds])];
    emit({ stage: 'dedup', level: 'info', message: `去重后候选 ${allIds.length} 篇`, progress: 36 });
    await sleep(200);
