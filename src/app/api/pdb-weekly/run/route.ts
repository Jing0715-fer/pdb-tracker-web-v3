 ...
 *
 * Mock of the pdb-tracker-web-v3 weekly report generator. Faithfully mirrors
 * the original SSE event stream + final `done` payload, including the
 * 1–3 cycle Generator / Critic-Scientific / Synthesis orchestration.
 */
...
function isoWeek(d: Date): { weekId: string; startDate: string; endDate: string; reportDate: string } {
  ...
  start.setUTCDate(tmp.getUTCDate() - 3);
  const end = new Date(start);
  end.setUTCDate(start.getUTCDate() + 6);
  const report = new Date(end);
  report.setUTCDate(end.getUTCDate() + 1);
...
export async function POST(req: Request) {
  ...
  (async () => {
    ...
    const cycles: any[] = [];
    const cycleRoles: Array<{ role: string; label: string; reportType: string }> = [
    ...
    for (let c = 1; c <= maxCycles; c++) {
      const role = cycleRoles[c - 1].role;
      const label = cycleRoles[c - 1].label;
      const reportType = cycleRoles[c - 1].reportType;
      ...
      // stream a few "thinking" sub-events
      for (let j = 1; j <= 3; j++) {
        await sleep(500);
        emit({
          stage: `cycle-${c}-${role}`,
          level: 'info',
          message: `C${c} ${label} · 推理 ${j}/3`,
          progress: baseProgress + Math.round((j / 3) * (60 / maxCycles) * 0.7),
        });
      }
      ...
      const cycleEntry = {
        ...
        provider,
        model,
        durationMs: Date.now() - cycleT0 + 3500,
        contentChars,
        verdict: role === 'critic-scientific' ? (Math.random() > 0.5 ? 'pass' : 'revise') : undefined,
      };
      cycles.push(cycleEntry);
      emit({
        stage: `cycle-${c}-${role}`,
        level: 'success',
        message: `C${c} ${label} 完成 · ${contentChars} chars${cycleEntry.verdict ? ` · verdict=${cycleEntry.verdict}` : ''}`,
        progress: baseProgress + Math.round((60 / maxCycles) * 0.95),
      });
    }
    ...
    const filesWritten = [
      `weekly-reports/${window.weekId}/cryoem.md`,
      `weekly-reports/${window.weekId}/xray.md`,
      `weekly-reports/${window.weekId}/index.md`,
    ];
    ...
    const result = {
      ...
      cycles,
      dbCounts: {
        pdbStructure: fetched,
        weeklyReport: maxCycles,
        weeklySnapshot: 1,
        withAuthors: 165,
        withPubmedId: 142,
        pubmedArticleMatched: 138,
